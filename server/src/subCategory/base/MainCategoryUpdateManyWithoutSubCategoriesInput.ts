/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { MainCategoryWhereUniqueInput } from "../../mainCategory/base/MainCategoryWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class MainCategoryUpdateManyWithoutSubCategoriesInput {
  @Field(() => [MainCategoryWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [MainCategoryWhereUniqueInput],
  })
  connect?: Array<MainCategoryWhereUniqueInput>;

  @Field(() => [MainCategoryWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [MainCategoryWhereUniqueInput],
  })
  disconnect?: Array<MainCategoryWhereUniqueInput>;

  @Field(() => [MainCategoryWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [MainCategoryWhereUniqueInput],
  })
  set?: Array<MainCategoryWhereUniqueInput>;
}

export { MainCategoryUpdateManyWithoutSubCategoriesInput as MainCategoryUpdateManyWithoutSubCategoriesInput };
