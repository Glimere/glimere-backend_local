/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ApparelCreateNestedManyWithoutSubSubcategoriesInput } from "./ApparelCreateNestedManyWithoutSubSubcategoriesInput";
import { ValidateNested, IsOptional, IsString } from "class-validator";
import { Type } from "class-transformer";
import { SubCategoryCreateNestedManyWithoutSubSubcategoriesInput } from "./SubCategoryCreateNestedManyWithoutSubSubcategoriesInput";

@InputType()
class SubSubcategoryCreateInput {
  @ApiProperty({
    required: false,
    type: () => ApparelCreateNestedManyWithoutSubSubcategoriesInput,
  })
  @ValidateNested()
  @Type(() => ApparelCreateNestedManyWithoutSubSubcategoriesInput)
  @IsOptional()
  @Field(() => ApparelCreateNestedManyWithoutSubSubcategoriesInput, {
    nullable: true,
  })
  apparel?: ApparelCreateNestedManyWithoutSubSubcategoriesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => SubCategoryCreateNestedManyWithoutSubSubcategoriesInput,
  })
  @ValidateNested()
  @Type(() => SubCategoryCreateNestedManyWithoutSubSubcategoriesInput)
  @IsOptional()
  @Field(() => SubCategoryCreateNestedManyWithoutSubSubcategoriesInput, {
    nullable: true,
  })
  subCategories?: SubCategoryCreateNestedManyWithoutSubSubcategoriesInput;
}

export { SubSubcategoryCreateInput as SubSubcategoryCreateInput };
